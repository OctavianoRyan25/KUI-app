...
@section('content')
        ...
        <ul class="inline-flex items-center space-x-1 rtl:space-x-reverse md:space-x-2">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <a href="{{ route('admin.researchCollaboration') }}">
                        <span class="text-gray-500 ms-1 text-sm font-medium hover:text-blue-600 md:ms-2">Research Collaboration</span>
                    </a>
                </div>
            </li>
        </ul>
    ...
    <div class="container mx-auto w-full px-4 py-8">
        <div class="row mb-4">
            <div class="col-md-12">
                <h1 class="font-bold text-3xl">Research Collaboration</h1>
            </div>
        </div>
        ...
        <div class="flex justify-end">
            <a href="/research-collaboration-reporting" id="openModalBtn" class="bg-[#003d7a] text-white mb-4 py-2 px-4 font-bold tracking-wider rounded-md transition duration-300 hover:bg-blue-600 hover:scale-105">
                Form
            </a>
        </div>
            ...
            <table class="min-w-full">
                <thead class="bg-[#003d7a] text-white text-sm font-bold tracking-wider text-center uppercase">
                    <tr>
                        ...
                        <th class="py-4 px-6">Koresponden</th>
                ...
                <tbody class="text-center">
                    @foreach ($researches as $index => $research)
                        <tr class="odd:bg-white even:bg-gray-300">
                            <td class="py-2 px-4">{{ $researches->firstItem() + $index }}</td>
                            <td class="py-2 px-4">{{ $research->correspondence }}</td>
                            <td class="py-2 px-4">{{ $research->study_program }}</td>
                            <td class="py-2 px-4">{{ $research->name }}</td>
                            <td class="py-2 px-4">{{ $research->email }}</td>
                            <td class="py-2 px-4">{{ $research->university }}</td>
                            <td class="py-2 px-4">
