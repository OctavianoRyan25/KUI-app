...
@section('content')
    <div class="container mx-auto p-6">
        ...
        <form action="{{ route('admin.event.store') }}" method="POST"
            class="w-full mx-auto bg-white p-8 rounded-lg shadow-lg">
            @csrf
            ...
            <div class="flex flex-wrap">
                ...
                <div class="mb-6 px-3 w-full lg:w-2/12">
                    ...
                    <input type="time" name="jam_rapat" id="jam_rapat"
                        class="mt-2 block w-full border border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2"
                        placeholder="e.g., 08.00-selesai" required aria-required="true">
                    @error('jam_rapat')
                        <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="flex justify-end gap-2">
                <a href="{{ route('admin.event') }}"
                    class="bg-red-500 text-white px-5 py-3 rounded-md shadow-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-[#003d7a] focus:ring-offset-2 transition duration-200 hover:ease-in-out hover:scale-105 font-bold">
                    Cancel
                </a>
                <button type="submit"
                    class="bg-[#003d7a] text-white px-5 py-3 rounded-md shadow-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-[#003d7a] focus:ring-offset-2 transition duration-200 hover:ease-in-out hover:scale-105 font-bold">
                    Create rapat
                </button>
            </div>
        </form>
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (data.length > 0) {
                                ...
                                $('#search-message').html(
                                    '<div class="p-2 text-gray-500">Peserta tidak ditemukan</div>'
                                );
                            }
                        },
                        error: function() {
                            $('#search-results').show();
                            $('#search-message').html(
                                '<div class="p-2 text-red-500">Error saat mencari peserta</div>'
                            );
                        }
                    });
                } else {
