                ...
                <div class="bg-white rounded-lg shadow-lg p-6">
                    <div class="row justify-start">
                        <div class="col-md-12 px-3">
                            <img src="{{ asset('assets/logo-udinus.png') }}"
                                class="font-bold text-3xl text-start h-16 w-16"></img>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-12">
                    ...
                    <div class="flex flex-wrap mt-6">
                        ...
                        <div class="mb-6 px-3 w-full lg:w-1/2">
                            <label for="tempat_rapat" class="block text-sm font-medium text-gray-700">Tempat/Media
                                Rapat</label>
                    ...
                    {{-- Show imgae if exist --}}
                    @if ($note->event->photos->isNotEmpty())
                        <div class="flex flex-wrap mt-6">
                            <div class="mb-6 px-3 w-full lg:w-3/4 flex items-center">
                                <label for="upload_photo" class="flex w-1/12 text-sm font-medium text-gray-700 ">Foto
                                    Rapat</label>
                                <div class="flex w-1/2 items-center gap-2">
                                    @foreach ($note->event->photos as $photo)
                                        <img src="{{ asset('storage/event_photos/' . $photo->photo_path) }}" alt="photo"
                                            class="w-1/4 h-1/4 rounded">
                                    @endforeach
                                </div>
                            </div>
                        </div>
                    @else
                        <form action="{{ route('admin.uploadPhoto', $note->event->id) }}" method="POST"
                            enctype="multipart/form-data">
                            <div class="flex flex-wrap mt-6">
                                <div class="mb-6 px-3 w-full lg:w-3/4 flex items-center">
                                    @csrf
                                    <label for="upload_photo" class="flex w-1/12 text-sm font-medium text-gray-700 ">Upload
                                        Foto</label>
                                    <div class="flex w-1/2 items-center gap-2">
                                        <input type="file" name="upload_photo[]" id="upload_photo"
                                            class="flex w-9/12 border border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm file:mr-4 file:rounded-md file:border-0 file:bg-teal-500 file:py-2 file:px-4 file:text-sm file:font-semibold file:text-white hover:file:bg-teal-700 focus:outline-none disabled:pointer-events-none disabled:opacity-60"
                    ...
                    @if ($note->event->files->isNotEmpty())
                        <div class="flex flex-wrap mt-6">
                            <div class="mb-6 px-3 w-full lg:w-3/4 flex items-center">
                                <label for="upload_file[]"
                                    class="flex w-1/12 text-sm font-medium text-gray-700">PPT</label>
                                <div class="flex w-1/2 items-center gap-2">
                                    @foreach ($note->event->files as $file)
                                        <a href="{{ asset('storage/event_files/' . $file->file_path) }}"
                                            class="px-4 py-2 bg-blue-500 text-white rounded-md hover:bg-blue-700 transition duration-200">
                                            Download
                                        </a>
                                    @endforeach
                    ...
                    @else
                        <form action="{{ route('admin.uploadFile', $note->event->id) }}" method="POST"
                            enctype="multipart/form-data">
                            @csrf
                            <div class="flex flex-wrap mt-6">
                                <div class="mb-6 px-3 w-full lg:w-3/4 flex items-center">
                                    <label for="upload_file[]"
                                        class="flex w-1/12 text-sm font-medium text-gray-700">Upload
                                        PPT</label>
                                    <div class="flex w-1/2 items-center gap-2">
                                        <input type="file" name="upload_file[]" id="upload_file[]"
                                            class="flex w-9/12 border border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm file:mr-4 file:rounded-md file:border-0 file:bg-teal-500 file:py-2 file:px-4 file:text-sm file:font-semibold file:text-white hover:file:bg-teal-700 focus:outline-none disabled:pointer-events-none disabled:opacity-60"
                                            multiple accept=".ppt,.pptx">
                                        <button type="submit"
                                            class="px-4 py-2 bg-blue-500 text-white rounded-md hover:bg-blue-700 transition duration-200">
                                            Upload
                                        </button>
                    ...
                    <div class="inline-flex gap-3 mx-3">
                        ...
                        <div class="mb-6 flex">
                            <button id="cancelButton"
                                class="px-4 py-2 bg-red-500 text-white rounded-md hover:bg-red-700 transition duration-200 hidden">Cancel</button>
                        </div>
                    </div>
                    {{-- batas --}}
                    <div class="mb-6 flex justify-between">
                        <div class="">
                            <p class="mb-6 px-3 inline-flex font-semibold">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 me-1 text-green-500"
                            ...
                            <p class="mb-6 px-3 inline-flex font-semibold">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 me-1 text-red-500"
                                ...
                                Tidak Hadir :{{ $no_present }}
                            </p>
                            <p class="mb-6 px-3 inline-flex font-semibold">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 me-1 text-blue-500"
                        ...
                        <div class="">
                            <p class="mb-6 px-3">Semarang,
                                {{ \Carbon\Carbon::parse($note->event->tanggal_rapat)->format('d M Y') }}</p>
