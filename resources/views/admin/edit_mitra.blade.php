...
@section('content')
  ...
  <main class="container w-full mx-auto py-8 px-4">
    ...
    {{-- ? error alert end --}}
    {{-- ? form start --}}
    <form action="{{ route('admin.mitra.update', ['id' => $mitra->id]) }}" method="POST" enctype="multipart/form-data" class="bg-white w-full mx-auto mb-4 p-8 rounded-md shadow-md">
      @method('PUT')
      @csrf
      {{-- ! mitra form start --}}
      <div class="flex flex-wrap">
        <div class="w-full mb-6 px-3 lg:w-1/2">
          ...
          <input type="text" name="nama_mitra" id="nama-mitra" placeholder="..." value="{{ old('nama_mitra', $mitra->nama_mitra) }}" class="w-full mt-2 p-2 block border border-gray-300 rounded-md shadow focus:outline-[#003d7a] sm:text-sm" autofocus>
          @error('nama_mitra')
        ...
        <div class="w-full mb-6 px-3 lg:w-1/4">
          ...
          <select name="kriteria_mitra" id="kriteria-mitra" class="w-full mt-2 p-2 block border border-gray-300 rounded-md shadow focus:outline-[#003d7a] sm:text-sm">
        ...
        <div class="w-full mb-6 px-3 lg:w-1/4">
          ...
          <select name="tingkat" id="tingkat" class="w-full mt-2 p-2 block border border-gray-300 rounded-md shadow focus:outline-[#003d7a] sm:text-sm">
        ...
        <div class="w-full mb-6 px-3 lg:w-1/4">
          ...
          <select name="regional" id="regional" class="w-full mt-2 p-2 block border border-gray-300 rounded-md shadow focus:outline-[#003d7a] sm:text-sm" disabled>
        ...
        <div class="w-full mb-6 px-3 lg:w-1/4">
          ...
          <select name="kota" id="kota" class="w-full mt-2 p-2 block border border-gray-300 rounded-md shadow focus:outline-[#003d7a] sm:text-sm" disabled>
        ...
        <div class="w-full mb-6 px-3 lg:w-1/4">
          ...
          <select name="negara" id="negara" class="w-full mt-2 p-2 block border border-gray-300 rounded-md shadow focus:outline-[#003d7a] sm:text-sm" disabled>
              ...
              @for ($i = 0; $i < 5; $i++)
                <tr class="odd:bg-white even:bg-gray-300">
                  <td class="py-2 px-3 border-[#003d7a] border-r">
                    <input type="text" name="mitra_kontaks[{{ $i }}][nama]" id="nama" value="{{ old('mitra_kontaks.' . $i . '.nama', $mitra->mitra_kontaks[$i]->nama ?? '') }}" class="bg-transparent w-full outline-none">
                  </td>
                  <td class="py-2 px-3 border-[#003d7a] border-x">
                    <input type="email" name="mitra_kontaks[{{ $i }}][email]" id="email" value="{{ old('mitra_kontaks.' . $i . '.email', $mitra->mitra_kontaks[$i]->email ?? '') }}" class="bg-transparent w-full outline-none">
                  </td>
                  <td class="py-2 px-3 border-[#003d7a] border-x">
                    <input type="text" name="mitra_kontaks[{{ $i }}][nomor_hp]" id="nomor-hp" value="{{ old('mitra_kontaks.' . $i . '.nomor_hp', $mitra->mitra_kontaks[$i]->nomor_hp ?? '') }}" class="bg-transparent w-full outline-none">
                  </td>
                  <td class="py-2 px-3 border-[#003d7a] border-x">
                    <input type="text" name="mitra_kontaks[{{ $i }}][nomor_telepon]" id="nomor-telepon" value="{{ old('mitra_kontaks.' . $i . '.nomor_telepon', $mitra->mitra_kontaks[$i]->nomor_telepon ?? '') }}" class="bg-transparent w-full outline-none">
                  </td>
                  <td class="py-2 px-3 border-[#003d7a] border-x">
                    <input type="text" name="mitra_kontaks[{{ $i }}][jabatan]" id="jabatan" value="{{ old('mitra_kontaks.' . $i . '.jabatan', $mitra->mitra_kontaks[$i]->jabatan ?? '') }}" class="bg-transparent w-full outline-none">
                  </td>
                  <td class="py-2 px-3 border-[#003d7a] border-l">
                    <input type="text" name="mitra_kontaks[{{ $i }}][alamat]" id="alamat" value="{{ old('mitra_kontaks.' . $i . '.alamat', $mitra->mitra_kontaks[$i]->alamat ?? '') }}" class="bg-transparent w-full outline-none">
                  </td>
                </tr>
              @endfor
            </tbody>
      ...
      <div class="flex flex-wrap">
        <div class="w-full px-3 flex justify-start space-x-2 sm:justify-end">
          ...
          <button type="submit" class="bg-[#003d7a] text-white mb-4 py-2 px-4 font-bold tracking-wider rounded-md transition duration-300 hover:bg-blue-600 hover:scale-105">Update</button>
        </div>
      </div>
      {{-- ! action buttons end --}}
    </form>
