...
<body class="bg-gradient-to-r from-blue-500 to-[#003d7a]">
    ...
    <div class="container mx-auto p-6">
        ...
        <!-- Form -->
        <form method="POST" action="{{ route('admin.researchCollaboration.store') }}" class="bg-white shadow rounded-lg p-6" enctype="multipart/form-data"onsubmit="handleFormSubmit(event)">
            @csrf
            ...
            <div class="mb-4 flex flex-wrap">
                ...
                <div class="w-full px-3 md:w-1/3">
                    <label for="journal_output" class="block text-sm font-medium text-gray-700">Luaran Jurnal<span class="text-red-500">*</span></label>
                    <select id="journal_output" name="journal_output" required class="mt-1 block w-full p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <div class="w-full px-3 md:w-1/3">
                    <label for="journal_level" class="block text-sm font-medium text-gray-700">TIngkatan Jurnal<span class="text-red-500">*</span></label>
                    <select id="journal_level" name="journal_level" required class="mt-1 block w-full p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <div class="mb-4 flex flex-wrap">
                <div class="w-full px-3 md:w-1/3">
                    ...
                    <input id="loa" type="file" name="loa" class="mt-2 block w-full text-sm file:mr-4 rounded-md file:rounded-md border file:border-0 file:bg-teal-500 file:py-2 file:px-4 file:text-sm file:font-semibold file:text-white hover:file:bg-teal-700 focus:outline-none disabled:pointer-events-none disabled:opacity-60" />
                    @error('loa')
                        <div class="text-red-500 text-sm mt-2">{{ $message }}</div>
                    @enderror
                </div>
                <div class="w-full px-3 md:w-1/3">
                    <label for="paper" class="text-sm font-bold text-gray-600">Paper<span class="text-red-500">*</span></label>
                    <input id="paper" type="file" name="paper" class="mt-2 block w-full text-sm file:mr-4 rounded-md file:rounded-md border file:border-0 file:bg-teal-500 file:py-2 file:px-4 file:text-sm file:font-semibold file:text-white hover:file:bg-teal-700 focus:outline-none disabled:pointer-events-none disabled:opacity-60" required/>
                    @error('paper')
                        <div class="text-red-500 text-sm mt-2">{{ $message }}</div>
                    @enderror
                </div>
                <div class="w-full px-3 md:w-1/3">
                    ...
                    <input id="invoice" type="file" name="invoice" class="mt-2 block w-full text-sm file:mr-4 rounded-md file:rounded-md border file:border-0 file:bg-teal-500 file:py-2 file:px-4 file:text-sm file:font-semibold file:text-white hover:file:bg-teal-700 focus:outline-none disabled:pointer-events-none disabled:opacity-60" />
                    @error('invoice')
                        <div class="text-red-500 text-sm mt-2">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="mb-4 flex flex-wrap">
                <div class="w-full px-3 ">
                    {{-- radio form --}}
                    <label for="is_PKS" class="block text-sm font-medium text-gray-700">Apakah bersedia PKS fakultas ke fakultas<span class="text-red-500">*</span></label>
                    <div class="mt-1 flex items-center">
                        <input id="is_PKS" name="is_PKS" type="radio" value="1" class="focus:ring-blue-500 h-4 w-4 text-blue-600 border-gray-300">
            ...
            <div class="mb-4 flex flex-wrap">
                <div class="w-full px-3 ">
                    <button id="submitButton" type="submit" class="bg-yellow-600 text-white px-4 py-3 rounded-lg hover:bg-yellow-700 transition duration-200 font-bold inline-flex" >
                        <span id="submitText">Submit</span>
                        <svg 
                            id="submitSpinner" 
                            class="hidden animate-spin ml-2 h-5 w-5 text-white mt-1" 
            ...
            if (query.length > 3) {
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.length > 0) {
                            data.forEach(item => {
                                dropdown.append(`<li data-name="${item.name}">${item.name}</li>`);
                            });
                        } else {
