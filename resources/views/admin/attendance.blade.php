...
<body class="bg-gradient-to-r from-blue-500 to-[#003d7a]">
    ...
    <div class="container mx-auto p-6">
        ...
        <div class="bg-white shadow-lg rounded-lg p-6 mb-6 border-t-4 border-blue-600 flex justify-between">
            <div class="">
                <h2 class="text-xl lg:text-2xl font-bold mb-2 text-blue-800">Absensi Rapat: {{ $event->nomor_rapat }}
                </h2>
                <p class="text-gray-700 text-sm lg:text-base">{{ $event->hal }} -
                    {{ \Carbon\Carbon::parse($event->tanggal_rapat)->format('d M Y') }}</p>
                <p class="text-gray-600 text-sm lg:text-base">{{ $event->tempat_rapat }} | Jam: {{ $event->jam_rapat }}
                </p>
            </div>
            <div class="content-center">
                <img src="{{ asset('assets/logo-udinus.png') }}" alt="Logo"
                    class="h-10 w-10 md:h-16 md:w-16 content-center">
            </div>
        </div>
        ...
        <div class="flex flex-wrap justify-between">
            <div class="mb-4 w-full lg:w-5/6 ">
                <input type="text" id="searchInput"
                    class="p-3 rounded-lg border border-blue-300 w-full active:border-blue-900"
                    placeholder="Cari peserta..." onkeyup="searchParticipant()">
            </div>
            <div class="mb-4 w-full lg:w-1/6 flex justify-end">
                <div class="w-full lg:ms-3 text-center">
                    <a class="bg-blue-600 text-white px-4 py-3 rounded-lg hover:bg-blue-700 transition duration-200 inline-flex w-full justify-center"
                        href="{{ route('admin.notulensi', $event->note->id) }}">
            ...
            <table class="min-w-full bg-white rounded-lg shadow-md">
                ...
                <tbody id="participantTable">
                    @foreach ($event->pesertas as $participant)
                        <tr class="border-b">
                            <td class="py-3 px-4 text-center text-xs md:text-base participant-name">
                                {{ $participant->name }}</td>
                            ...
                            <td class="py-3 px-4 justify-items-center items-center">
                                @if ($participant->pivot->signature)
                                    <img src="{{ $participant->pivot->signature }}" draggable="false"
                                        oncontextmenu="return false;" alt="Signature"
                                        class="h-10 w-auto border border-gray-300 rounded">
                                @else
                                    <div class="flex justify-center">
                                        <button
                                            class="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition duration-200"
                                            data-participant-id="{{ $participant->id }}"
                                            data-name="{{ $participant->name }}" data-nip="{{ $participant->nip }}"
        ...
        <div id="signaturePadModal" class="fixed z-10 inset-0 overflow-y-auto hidden bg-black bg-opacity-50">
            <div class="flex items-center justify-center min-h-screen px-4">
                <div class="bg-white p-6 rounded-lg shadow-lg relative max-w-md w-full overflow-auto">
                    ...
                    <div class="flex flex-wrap">
                        <label for="signature" class="block text-sm font-medium text-gray-700 mb-3">Tanda
                            tangan</label>
                        <p
                            class="text-sm text-red-500 bg-red-50 border border-red-200 rounded-md px-3 py-2 w-full mb-6 flex items-center">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 me-1" fill="currentColor"
                        ...
                        <canvas id="signaturePad" width="300" height="150"
                            class="border rounded-md border-gray-300 mb-6 px-3"></canvas>
                    </div>
